`
🤖 本脚本转换自：https://greasyfork.org/scripts/406849-vip-视频解析-优化增强版/code/VIP 视频解析(优化增强版).user.js。
...
(function () {
    ...
    switch (host) {
        ...
        console.log("[油猴脚本] VIP 视频解析(优化增强版).user.js 注入成功!");
    }
    $done({body});
} catch (err) {
    console.log("[油猴脚本] VIP 视频解析(优化增强版).user.js 执行失败!\n" + err);
    $done({});
}
	◦	
